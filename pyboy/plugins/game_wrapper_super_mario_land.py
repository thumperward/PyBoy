...
class GameWrapperSuperMarioLand(PyBoyGameWrapper):
    ...
    def post_tick(self):
        ...
        if self.game_has_started:
            ...
            self.fitness = (
                self.lives_left * 10000
                + end_score
                + self._level_progress_max * 10
            )
    ...
    def start_game(
        self, timer_div=None, world_level=None, unlock_level_select=False
    ):
        """
        ...
        while True:
            self.pyboy.tick()
            if self.tilemap_background[6:11, 13] == [
                284,
                285,
                266,
                283,
        ...
        while True:
            ...
            if unlock_level_select and self.pyboy.frame_count == 71:
                self.pyboy.set_memory_value(
                    ADDR_WIN_COUNT, 2 if unlock_level_select else 0
                )
                break
            self.pyboy.tick()
            ...
            if (
                self.tilemap_background[0:5, 0] == [278, 266, 283, 274, 280]
                and self.tilemap_background[5, 1] == 256
            ):
