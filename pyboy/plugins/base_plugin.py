...
class PyBoyGameWrapper(PyBoyPlugin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tilemap_background = (
            self.pyboy.botsupport_manager().tilemap_background()
        )
    ...
    def enabled(self):
        return (
            self.pyboy_argv.get("game_wrapper")
            and self.pyboy.cartridge_title() == self.cartridge_title
        )
    ...
    def _game_area_tiles(self):
        if self._tile_cache_invalid:
            ...
            if self.game_area_wrap_around:
                self._cached_game_area_tiles = np.ndarray(
                    shape=(height, width), dtype=np.uint32
                )
                for y in range(height):
                    ...
                    for x in range(width):
                        ...
                        self._cached_game_area_tiles[y][
                            x
                        ] = self.tilemap_background.tile_identifier(_x, _y)
            else:
    ...
    def _game_area_np(self, observation_type="tiles"):
        if observation_type == "tiles":
            ...
            try:
                return self.tiles_compressed[
                    np.asarray(self.game_area(), dtype=np.uint16)
                ]
    ...
    def _sum_number_on_screen(
        self, x, y, length, blank_tile_identifier, tile_identifier_offset
    ):
        return sum(
