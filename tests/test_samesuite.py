...
@pytest.mark.parametrize("clean, gb_type, rom", [
...
def test_samesuite(clean, gb_type, rom, samesuite_dir, boot_cgb_rom, boot_rom,
                   default_rom):
    global saved_state
    ...
    for _ in range(10):
        if not np.all(
                pyboy.botsupport_manager().screen().screen_ndarray() > 240):
            break
