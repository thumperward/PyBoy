...
def test_argv_parser(*args):
    ...
    assert parser.parse_args(f"{file_that_exists} --loadstate".split(
        " ")).loadstate == main.INTERNAL_LOADSTATE
    ...
    flags = parser.parse_args(
        f"{file_that_exists} --debug --autopause --profiling --rewind --no-input --log-level INFO"
        .split(" ")).__dict__
    for k, v in {
            "autopause": True,
            "debug": True,
...
def test_randomize_ram(default_rom):
    ...
    assert not any(pyboy.get_memory_value(x)
                   for x in range(0xC000, 0xE000)), "Internal RAM 0 not zeroed"
    assert not any(pyboy.get_memory_value(x)
                   for x in range(0xFE00, 0xFEA0)), "OAM not zeroed"
    assert not any(
        pyboy.get_memory_value(x)
        for x in range(0xFEA0, 0xFF00)), "Non-IO internal RAM 0 not zeroed"
    assert not any(
        pyboy.get_memory_value(x)
        for x in range(0xFF4C, 0xFF80)), "Non-IO internal RAM 1 not zeroed"
    assert not any(pyboy.get_memory_value(x)
                   for x in range(0xFF80, 0xFFFF)), "Internal RAM 1 not zeroed"
    pyboy.stop(save=False)
    ...
    assert any(pyboy.get_memory_value(x)
               for x in range(0xC000, 0xE000)), "Internal RAM 0 not randomized"
    assert any(pyboy.get_memory_value(x)
               for x in range(0xFE00, 0xFEA0)), "OAM not randomized"
    assert any(
        pyboy.get_memory_value(x)
        for x in range(0xFEA0, 0xFF00)), "Non-IO internal RAM 0 not randomized"
    assert any(
        pyboy.get_memory_value(x)
        for x in range(0xFF4C, 0xFF80)), "Non-IO internal RAM 1 not randomized"
    assert any(pyboy.get_memory_value(x)
               for x in range(0xFF80, 0xFFFF)), "Internal RAM 1 not randomized"
    pyboy.stop(save=False)
...
def test_not_cgb(pokemon_crystal_rom):
    ...
    assert pyboy.botsupport_manager().tilemap_background()[1:16, 16] == [
        134, 160, 172, 164, 383, 129, 174, 184, 383, 130, 174, 171, 174, 177,
        232
...
@pytest.mark.parametrize("_bootrom, frames",
                         [(pytest.lazy_fixture("boot_cgb_rom"), 120),
                          (pytest.lazy_fixture("boot_rom"), 120), (None, 30)])
@pytest.mark.parametrize(
    "rom",
    [pytest.lazy_fixture("tetris_rom"),
     pytest.lazy_fixture("any_rom_cgb")])
def test_all_modes(cgb, _bootrom, frames, rom, any_rom_cgb, boot_cgb_rom):
    ...
    png_path = Path(
        f"test_results/all_modes/{rom_name}_{cgb}_{os.path.basename(str(_bootrom))}.png"
    )
    ...
    if OVERWRITE_PNGS:
        ...
        with open(png_path, "wb") as f:
            f.write(b"".join([(x ^ 0b10011101).to_bytes(1, sys.byteorder)
                              for x in png_buf.getvalue()]))
    else:
        png_buf = BytesIO()
        with open(png_path, "rb") as f:
            data = f.read()
            png_buf.write(b"".join([
                (x ^ 0b10011101).to_bytes(1, sys.byteorder) for x in data
            ]))
        png_buf.seek(0)
        ...
        ), f"Images are different! {(cgb, _bootrom, frames, rom)}"
