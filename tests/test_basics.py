...
def test_record_replay(boot_rom, default_rom):
    ...
    assert (
        len(events) == 4
    ), "We assumed only 4 frames were recorded, as frames without events are skipped."
    frame_no, keys, frame_data = events[0]
    assert frame_no == 1, "We inserted the key on the second frame"
    assert (
        keys[0] == WindowEvent.PRESS_ARROW_DOWN
    ), "Check we have the right keypress"
    assert (
        sum(base64.b64decode(frame_data)) / 0xFF == 144 * 160 * 3
    ), "Frame does not contain 160x144 of RGB data"
    ...
    assert (
        digest
        == b"\xc0\xfe\x0f\xaa\x1b0YY\x1a\x174\x8c\xad\xeaDZ\x1dQ\xa8\xa2\x9fA\xaap\x15(\xc9\xd9#\xd4]{"
...
def test_profiling(boot_rom, default_rom):
    pyboy = PyBoy(
        default_rom, window_type="dummy", bootrom_file=boot_rom, profiling=True
    )
    pyboy.set_emulation_speed(0)
    pyboy.tick()
    ...
    CHECK_SUM = 7524
    assert (
        sum(hitrate) == CHECK_SUM
    ), "The amount of instructions called in the first frame of the boot-ROM has changed"
...
def test_argv_parser(*args):
    ...
    assert (
        parser.parse_args(
            f"{file_that_exists} --loadstate".split(" ")
        ).loadstate
        == main.INTERNAL_LOADSTATE
    )
    assert (
        parser.parse_args(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pytest.lazy_fixture("boot_rom"), 120),
        (None, 30),
    ],
)
@pytest.mark.parametrize(
    "rom",
    [pytest.lazy_fixture("tetris_rom"), pytest.lazy_fixture("any_rom_cgb")],
)
    ...
    if OVERWRITE_PNGS:
        ...
        with open(png_path, "wb") as f:
            f.write(
                b"".join(
                    [
                        (x ^ 0b10011101).to_bytes(1, sys.byteorder)
