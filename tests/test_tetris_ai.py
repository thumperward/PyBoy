...
@pytest.mark.skipif(
    os.path.isfile("README/7.gif") or platform.system() == "Windows",
...
def test_tetris_ai(git_tetris_ai, tetris_rom):
    ...
    assert (
        os.system(f"cp {tetris_rom} {Path(git_tetris_ai) / 'tetris_1.1.gb'}")
        == 0
    )
    assert (
        os.system(
            f'cd {git_tetris_ai} && . {Path(".venv") / "bin" / "activate"} && python {script_py}'
        )
        == 0
    )
    assert (
        os.system(
            f'mv {Path(git_tetris_ai) / "recordings" / "TETRIS*"} {Path("README/7.gif")}'
        )
        == 0
    )
