...
class TestLCD:
    ...
    def test_stat_register(self):
        ...
        lcd.set_lcdc(
            0b1000_0000
        )  # Turn on LCD. Don't care about rest of the flags
        ...
        assert (
            lcd.get_stat() & 0b1000_0000
        ) == 0x80  # LCD on bit (read-only) will not be affected
        assert (lcd.get_stat() & 0b0000_0111) == 0b000

        assert (
            lcd.get_stat() & 0b11
        ) == 0b00  # Check that we can read out screen mode
        lcd._STAT.set_mode(2)
        assert (
            lcd.get_stat() & 0b11
        ) == 0b10  # Check that we can read out screen mode
    ...
    def test_check_lyc(self):
        ...
        assert (
            lcd._STAT.update_LYC(lcd.LYC, lcd.LY) == 0
        ...
        assert (
            lcd._STAT.update_LYC(lcd.LYC, lcd.LY) == 0
        )  # LYC!=LY, and no interrupts enabled
        ...
        assert not (lcd.get_stat() & 0b100)  # LYC flag not set
        assert (
            lcd._STAT.update_LYC(lcd.LYC, lcd.LY) == INTR_LCDC
        )  # Trigger on seting LYC flag
        assert (
            lcd._STAT.update_LYC(lcd.LYC, lcd.LY) == INTR_LCDC
        )  # Also trigger on second call
        assert lcd.get_stat() & 0b100  # LYC flag set
